...
# Initialize DB first to ensure config table exists
init_db()
init_msg_db()

# Priority: ENV -> DB -> Generate
SECRET_KEY = os.getenv("JWT_SECRET_KEY")
if not SECRET_KEY:
    SECRET_KEY = get_jwt_secret_db()
    if not SECRET_KEY:
        SECRET_KEY = secrets.token_urlsafe(32)
        set_jwt_secret_db(SECRET_KEY)
        print("🔐 Generated new persistent JWT Secret.")
...
def verify_admin(x_admin_key: str = Header(None)):
    ...
    try:
        is_valid = secrets.compare_digest(
            clean_key.encode("utf-8"), clean_secret.encode("utf-8")
        )
    ...
    if not is_valid:
        ...
        masked_key = (
            clean_key[:4] + "..." + clean_key[-4:] if len(clean_key) > 8 else "****"
        )
        print(
            f"🚫 Admin Access Denied: Key mismatch. Received length: {len(clean_key)} (Expected: {len(clean_secret)}), Masked input: {masked_key}"
        )
...
async def handle_delete_account(
    user: UserAuth, current_user: str = Depends(get_current_user)
):
    # 1. Verify Password first (Security Check)
    # Ensure current_user is deleting their own account
    ...
    auth_status = verify_user(
        user.username,
        user.user_password if hasattr(user, "user_password") else user.password,
    )
...
app.include_router(
    profiles.router, prefix="/profile", tags=["Profile"]
)  # 👈 3. Add Profile Router
