...
def save_message(
    text, sender, receiver, msg_type="text", file_url=None, file_name=None, options=None
):
    # ✅ IST timestamp
...
def create_delivery_entries(msg_id, recipients: list):
    ...
    data = [(msg_id, user) for user in recipients]
    cur.executemany(
        """
...
def cleanup_old_messages(days: int):
    ...
    cur.execute(
        "DELETE FROM delivery_status WHERE msg_id NOT IN (SELECT id FROM messages)"
    )
...
def save_user_fact(username: str, fact: str):
    ...
    cur.execute(
        "INSERT INTO user_facts (username, fact, ts) VALUES (?, ?, ?)",
        (username, fact, ts),
    )
...
def get_user_facts(username: str):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT fact FROM user_facts WHERE username = ? ORDER BY ts ASC", (username,)
    )
    rows = cur.fetchall()
    conn.close()
